...
dataset_path = {
    "example": "tests/testdata/example",
    "complete": "tests/testdata/metadata_Supervisor_20250108_101446_62_cm40593-1_EPU",
}
...
class GridSquareParser:
    ...
    def parse(self) -> GridSquare:
        ...
        if xml_files:
            try:
                timestamp = parse_timestamp(xml_files[0].stem)
            except ValueError:
...
class EPUSessionParser:
    ...
    def parse(self) -> EPUSession:
        ...
        microscope_settings = ns_mgr.find(root, 'MicroscopeSettings')
        if microscope_settings is not None:
            for setting in microscope_settings.findall('.//*[DisplayName]'):
                display_name = ns_mgr.find(setting, 'DisplayName')
                if display_name is not None and display_name.text:
                    # Extract relevant settings (customize based on needs)
                    settings_dict = {
                        'exposure_time': float(ns_mgr.find(setting, './/ExposureTime').text) if ns_mgr.find(setting,
                                                                                                            './/ExposureTime') is not None else None,
                        'defocus': float(ns_mgr.find(setting, './/Defocus').text) if ns_mgr.find(setting,
                                                                                                 './/Defocus') is not None else None,
                    }
        ...
        for disc_dir in image_discs:
            ...
            for grid_square_dir in disc_path.iterdir():
                if not grid_square_dir.is_dir() or not grid_square_dir.name.startswith('GridSquare_'):
                    continue

                grid_square_id = grid_square_dir.name.split('_')[1]
                metadata_file = metadata_dir / f"GridSquare_{grid_square_id}.dm"

                if not metadata_file.exists():
                    print(f"Warning: No metadata file found for {grid_square_dir.name}")
                    continue

                parser = GridSquareParser(grid_square_dir, metadata_file)
...
def parse_example():
    dir_path = dataset_path["example"]
    if not os.path.exists(dir_path):
        print(f"Error: Could not find EPU session dir at: {dir_path}")
        return

    parser = EPUSessionParser(dir_path)
    try:
...
def parse_real_dataset():
    dir_path = dataset_path["complete"]
    if not os.path.exists(dir_path):
        print(f"Error: Could not find EPU session dir at: {dir_path}")
        return
